  ...
  return (
    <>
      ...
      <Modal visible={visible} transparent animationType="none" onRequestClose={closePicker}>
        {/* 背景遮罩 */}
        <Pressable className="flex-1 bg-black/50" onPress={closePicker} />

        {/* 底部卡片 */}
        <Animated.View
          style={{ transform: [{ translateY: slideAnim }] }}
          className="absolute bottom-0 left-0 right-0 rounded-t-3xl border-t border-neutral-700 bg-neutral-900">
                  ...
                  return (
                    <Pressable
                      ...
                      <View className="flex-1">
                        ...
                        {model.display_name && (
                          <ThemedText className="mt-0.5 font-mono text-[11px] text-neutral-500">
                            {model.id}
                          </ThemedText>
                        )}
                      </View>
                      {isActive && <Icon name="Check" size={16} color="#60a5fa" />}
                    </Pressable>
                  );
                })}
              </ScrollView>
