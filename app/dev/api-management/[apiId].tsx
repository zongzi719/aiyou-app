  ...
  return (
    <button
      ...
      style={{
        ...
        border: 'none',
        cursor: 'pointer',
        background: checked ? '#3b82f6' : '#404040',
        position: 'relative',
...
export default function ApiDetail() {
  ...
  if (!api) {
    return (
      <div style={{ padding: 24, color: '#737373' }}>
        接口不存在。
        <button
          onClick={() => router.push('/dev/api-management')}
          style={{
            color: '#60a5fa',
            background: 'none',
            border: 'none',
  ...
  const hasQueryParams =
    Object.keys(api.queryParams).length > 0 && Object.values(api.queryParams).some((v) => v !== '');
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      try {
        extraHeaders = testHeaders ? JSON.parse(testHeaders) : {};
      } catch {}
      let path = api.path;
      if (testQuery) {
        try {
          path += `?${new URLSearchParams(JSON.parse(testQuery)).toString()}`;
        } catch {}
      }
      ...
      const { status, text, time } = await apiRelayWebRaw({
        path,
        method: api.method,
  ...
  const statusLabel =
    usage?.status === 'connected'
      ...
      : usage?.status === 'partial'
        ...
        : '❌ 未对接';
  const statusColor =
    usage?.status === 'connected' ? '#4ade80' : usage?.status === 'partial' ? '#facc15' : '#f87171';
    ...
    <div style={{ padding: 24, maxWidth: 900, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span
          ...
          {groupLabels[api.group] || api.group}
        </span>
      </div>
      ...
      <div style={card}>
        {[
          { label: '路径', val: api.path },
          { label: '基础URL', val: baseUrl },
          { label: '描述', val: api.description },
          ...(api.notes ? [{ label: '备注', val: api.notes }] : []),
        ].map((row) => (
          <div key={row.label} style={{ display: 'flex', gap: 24, fontSize: 13, marginBottom: 8 }}>
            ...
            <span
              style={{
                color: row.label === '备注' ? '#fbbf24' : '#d4d4d4',
                fontFamily: 'monospace',
              }}>
              {row.val}
            </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {usage.usages.map((u, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, fontSize: 12 }}>
                <span style={{ fontFamily: 'monospace', color: '#737373', flexShrink: 0 }}>
                  {u.file}
                </span>
      ...
      {tab === 'docs' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {hasPathParams && (
            <div>
              <div style={{ fontSize: 13, fontWeight: 500, color: '#a3a3a3', marginBottom: 8 }}>
                Path 参数
              </div>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 500, color: '#4ade80', marginBottom: 8 }}>
              成功返回
            </div>
      ...
      {tab === 'test' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ fontSize: 11, color: '#737373', marginBottom: 4, display: 'block' }}>
              Headers (JSON)
            </label>
            <textarea
              value={testHeaders}
              onChange={(e) => setTestHeaders(e.target.value)}
              placeholder={JSON.stringify(api.headers, null, 2)}
              style={textarea}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              placeholder={hasQueryParams ? JSON.stringify(api.queryParams, null, 2) : '{}'}
              style={textarea}
            />
          </div>
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: 8, fontSize: 13 }}>
                <span
                  style={{
                    fontWeight: 700,
                    color:
                      testResult.status >= 200 && testResult.status < 400 ? '#4ade80' : '#f87171',
                  }}>
